    ...
    <form id="parallax-create" method="post" action="<?php echo admin_url(); ?>admin-post.php">
        ...
        if (isset($_GET['id'])) {
            ...
            if (!empty($itemQuery)) {
                ...
                echo $bt_utility->getMessage(__('No item with id (' . $_GET['id'] . ') found. Please add new slider', 'gmswebdesign'), 'notice');
            }
        }
        ?>
        <div class="create-new-wrap">
            ...
            if (isset($_GET['content_type'])):
                ...
                if (in_array($content_type, array('video-background', 'image-gallery', 'woo-commerce', 'wordpress-posts'))):
                    ...
                    <div class="tabs">
                        <ul class="tab-links">
                            <li class="active"><a href="#parallax-slide"><i class="fa fa-picture-o"></i>&nbsp;&nbsp;&nbsp;<?php echo __('PARALLAX SLIDER', 'gmswebdesign'); ?></a></li>
                            <?php if ($content_type == 'image-gallery'): ?>
                                <li class="dynamic-tab <?php echo ($item->settings->parallax_bg_type && $item->settings->parallax_bg_type != 'dynamic' ) ? 'hidden' : ''; ?>"><a href="#images"><i class="fa fa-cog"></i>&nbsp;&nbsp;&nbsp;<?php echo __('IMAGE GALLERY SETTING', 'gmswebdesign'); ?></a></li>
                            <?php endif; ?>
                            <?php if ($content_type == 'video-background'): ?>
                                <li class="dynamic-tab <?php echo ($item->settings->parallax_bg_type && $item->settings->parallax_bg_type != 'dynamic' ) ? 'hidden' : ''; ?>"><a href="#videos"><?php echo __('VIDEO SETTING', 'gmswebdesign'); ?></a></li>
                            <?php endif; ?>
                            <?php if ($content_type == 'woo-commerce'): ?>
                                <li class="dynamic-tab <?php echo ($item->settings->parallax_bg_type && $item->settings->parallax_bg_type != 'dynamic' ) ? 'hidden' : ''; ?>"><a href="#woo-commerce"><?php echo __('WOO COMMERCE SETTING', 'gmswebdesign'); ?></a></li>
                            <?php endif; ?>
                            <?php if ($content_type == 'wordpress-posts'): ?>
                                <li class="dynamic-tab <?php echo ($item->settings->parallax_bg_type && $item->settings->parallax_bg_type != 'dynamic' ) ? 'hidden' : ''; ?>"><a href="#wp-posts"><?php echo __('POST SETTING', 'gmswebdesign'); ?></a></li>
                            <?php endif; ?>
                        </ul>
        ...
        <?php if (isset($_GET['content_type']) && in_array($_GET['content_type'], array('video-background', 'image-gallery', 'woo-commerce', 'wordpress-posts'))): ?>
            <div class="control-buttons">
                ...
                <div class="create-button">
                    ...
                    <span class="button blue" onclick="javascript:document.getElementsByName('after_save')[0].value = '';document.forms['parallax-create'].submit();"><i class="fa fa-plus"></i> <?php echo __('SAVE SLIDESHOW', 'gmswebdesign'); ?></span>
                    <span class="button blue" onclick="javascript:document.getElementsByName('after_save')[0].value = 'return'; document.forms['parallax-create'].submit();"><i class="fa fa-plus"></i> <?php echo __('SAVE & CLOSE', 'gmswebdesign'); ?></span>
                    <span class="button red" onclick="javascript:window.location.href = btAdvParallaxBackgroundCfg.siteUrl + 'admin.php?page=bt-advance-parallax-background'"><i class="fa fa-times"></i> <?php echo __('CANCEL', 'gmswebdesign'); ?></span>
                </div>
