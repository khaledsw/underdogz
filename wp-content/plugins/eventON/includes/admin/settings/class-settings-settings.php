...
class evo_settings_settings{
	...
	function content(){
		...
		return apply_filters('eventon_settings_tab1_arr_content', array(
			...
			,array(
				...
				'fields'=>array(
					...
					array('id'=>'evo_ftim_mag','type'=>'yesno','name'=>__('Show magnifying glass over featured image','eventon'),'legend'=>'This will convert the mouse cursor to a magnifying glass when hover over featured image. <br/><br/><img src="'.AJDE_EVCAL_URL.'/assets/images/ajde_backender/cursor_mag.jpg"/>'),
		...
		function export(){
			...
			<p><a class='evo_admin_btn btn_triad' href="<?php echo $exportURL;?>"><?php _e('Download All Events (CSV)','eventon');?></a> <?php echo $eventon->frontend->get_html_guide('Download all eventON calendar events as CSV file.','eventon');?></p>
			<?php 
			return  ob_get_clean();
		}
	...
	// rearrange fields
		function rearrange_code(){			
			...
			$_saved_order = (!empty($this->evcal_opt[1]['evoCard_order']))? 
				...
				:null;
			$_hide_items = (!empty($this->evcal_opt[1]['evoCard_hide']))? 
				array_filter(explode(',',$this->evcal_opt[1]['evoCard_hide']))
				:null;
			
			// HTML
			$_rearrange_code = '<h4 class="acus_header">'.__('Re-arrange the order of eventCard event data boxes','eventon').'</h4>
				<p><i>'.__('NOTE: You can hide each data field for eventcard using the button on the left side of field name below. Grey = hide, Ogrange = show','eventon').'</i></p>
				...
				<input id="evoCard_hide" type="hidden" name="evoCard_hide" value="'.( (!empty($this->evcal_opt[1]['evoCard_hide']))? $this->evcal_opt[1]['evoCard_hide']:null).'"/>
				<div id="evoEVC_arrange_box">';
					...
					if($_saved_order){
						...
						foreach($_saved_order as $box){
							$_rearrange_code .= (array_key_exists($box, $rearrange_items))? 
								"<p val='".$rearrange_items[$box][0]."'><span class='". ( !empty($_hide_items) && in_array($box, $_hide_items)?'hide':'') ."'></span>".$rearrange_items[$box][1]."</p>":
								null;
						}	
						...
						if(count($_saved_order) < count($rearrange_items)){						
							foreach($rearrange_items as $f=>$v){
								$_rearrange_code .= (!in_array($f, $_saved_order))? 
									"<p val='".$rearrange_items[$f][0]."'><span class='". ( !empty($_hide_items) && in_array($box, $_hide_items)?'hide':'') ."'></span>".$rearrange_items[$f][1]."</p>"
									:null;
							}
						}						
					}
					...
					if(empty($rearrange_items) || empty($_saved_order)){	
						foreach($rearrange_items as $item){
							$_rearrange_code .= "<p val='".$item[0]."'><span class='". ( !empty($_hide_items) && in_array($box, $_hide_items)?'hide':'') ."'></span>".$item[1]."</p>";
						}				
					}
	...
	function content_shortcodes(){
		...
		?>
			...
			<a id="evo_shortcode_btn" class="eventon_popup_trig evo_admin_btn btn_prime" title="eventON Shortcode generator" href="#" data-textbox='evo_set_shortcodes'>[ ] <?php _e('Generate shortcode','eventon');?></a><br/>
