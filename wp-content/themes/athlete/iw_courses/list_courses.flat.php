<section class="classes" id="classes">
    ...
    if ($show_filter_bar):
        ...
        <div class="categories">
            <div class="filters button-group" id="filters">
                <?php
                if (!empty($categories)) {
                    echo '<button class="filter is-checked" data-filter="*">' . __('All Class', 'gmswebdesign') . '</button>';
                                            ...
                                            <div class="table">
                                                <div class="box-cell">
                                                    ...
                                                    <div class="box-details">
                                                        <a href="<?php echo get_the_permalink(); ?>"><?php echo __('Details', 'gmswebdesign'); ?></a>
                                                    </div>
                                                </div>
                                            </div>	
                                        </div>
    ...
    <div class="load-product">
        ...
        if ($rs['success']) {
            echo '<button class="load-more load-courses" id="load-more-class"><span class="ajax-loading-icon" style="margin-right: 10px; display: none;"><i class="fa fa-spinner fa-spin fa-2x"></i></span>' . __('Load More', 'gmswebdesign') . '</button>';
            echo $rs['data'];
        } else {
            echo '<button class="load-more load-courses all-loaded" id="load-more-class"><span class="ajax-loading-icon" style="margin-right: 10px; display: none;"><i class="fa fa-spinner fa-spin fa-2x"></i></span>' . __('All loaded', 'gmswebdesign') . '</button>';
        }
        wp_reset_postdata();
        ?>
    </div>
