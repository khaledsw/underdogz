	...
	function eventon_admin_scripts() {
		...
		if( (!empty($pagenow) && $pagenow=='admin.php')
		 && (!empty($_GET['page']) && $_GET['page']=='eventon'|| $_GET['page']=='action_user') 
			...
			wp_enqueue_script('evcal_backend_all',AJDE_EVCAL_URL.'/assets/js/eventon_all_backend.js',array('jquery'),$eventon->version,true);
			wp_enqueue_script('evcal_backend',AJDE_EVCAL_URL.'/assets/js/eventon_backend.js',array('jquery'),$eventon->version,true);		
